...
def create_app(test_config=None):
    ...
    def get_categories():
        try:
            ...
            categories_format = {category.format()['id']: category.format()[
    ...
    def get_questions():
        try:
            ...
            currentCategoryId = Question.query.filter(
                Question.question == current_questions[0]["question"]).first().category
            currentCategory = Category.query.filter(
                Category.id == currentCategoryId).first().type
        ...
        try:
            ...
            if search_term:
                ...
                if len(selections) == 0:
                    ...
                    # # current category is the category of the first question
                    currentCategoryId = Question.query.filter(
                        Question.question == formatted_question[0]["question"]).first().category
                    currentCategory = Category.query.filter(
                        Category.id == currentCategoryId).first().type

                    return jsonify({"success": True, "questions": questions, "totalQuestions": len(
                        formatted_question), "currentCategory": currentCategory})
            else:
    ...
    def get_quiz():
        try:
            ...
            possible_question = [
                f_q for f_q in format_selected if f_q["id"] not in previous_questions]
